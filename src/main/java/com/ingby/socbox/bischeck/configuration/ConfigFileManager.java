...
public class ConfigFileManager {
    ...
    // A cache of JAXBContext object used so they are not created on every
    // reload
    private static ConcurrentMap<String, JAXBContext> jccache = new ConcurrentHashMap<String, JAXBContext>();

    /**
     * Check if the property bishome is set and if the directory for the
     * configuration files exists and is readable. This check is done depending
     * on the sytem property xmlconfigdir. If not set:<br>
     * $bishome/etc<br>
     * or if set:<br>
     ...
     *             if the configuration directory do not exist or if the
    ...
    static public File initConfigDir() throws ConfigurationException {
        ...
        if (System.getProperty("bishome") == null) {
            ...
            throw new ConfigurationException(
                    "System property bishome must be set");
        ...
        if (configDir.isDirectory() && configDir.canRead()) {
            ...
            LOGGER.warn(
                    "Configuration directory {} does not exist or is not readable.",
                    configDir.getPath());
            throw new ConfigurationException("Configuration directory "
                    + configDir.getPath()
     ...
     * 
     * @param xmlconf
     *            member of the enum {@link ConfigXMLInf.XMLCONFIG}
     * @return return the object to the JAXB generated class based on the
     *         configuration files xsd schema files.
     * @throws ConfigurationException
     *             if any errors when manage the xml configuration file, like
     *             don't exist, can not be read, is not valid, missing xsd file,
     *             etc.
     */
    synchronized public Object getXMLConfiguration(XMLCONFIG xmlconf)
            throws ConfigurationException {
        Object xmlobj = null;

        xmlobj = createXMLConfig(xmlconf, ConfigFileManager.initConfigDir()
                .getPath());
        return xmlobj;
    }
     ...
     *            member of the enum {@link ConfigXMLInf.XMLCONFIG}
     * @param directory
     *            the directory location where to find the config file
     * @return return the object to the JAXB generated class based on the
     *         configuration files xsd schema files.
     * @throws ConfigurationException
     *             if any errors when manage the xml configuration file, like
    ...
    /**
     * Method manage the process to read xml config file and manage it into a
     * XMLxxx objects.
     * 
     * @param xmlconf
     * @param directory
     ...
     *             etc.
     */
    private Object createXMLConfig(XMLCONFIG xmlconf, String directory)
            throws ConfigurationException {
        Object xmlobj = null;
        ...
        if (jc == null) {
            try {
                ...
                LOGGER.error("Could not get JAXB context for {}",
                        xmlconf.instance(), e);
        ...
        URL xsdUrl = ConfigurationManager.class.getClassLoader().getResource(
                xmlconf.xsd());
        if (xsdUrl == null) {
            LOGGER.error("Could not find xsd file {} in classpath",
                    xmlconf.xsd());
            throw new ConfigurationException("Could not find xsd file "
        ...
        try {
            ...
            LOGGER.error(
                    "Could not create set event handler vector for unmarshaller for {}",
                    xmlconf.instance(), e);
            throw new ConfigurationException(e);
        }
        ...
        try {
            ...
            if (vec.hasEvents()) {
                ...
                for (ValidationEvent event : vec.getEvents()) {
                    ...
                    strbuf.append(" - Error at line " + line + " column "
                            + column);
                }
                LOGGER.error(
                        "Could not unmarshall the file {} - {}" + xmlconf.xml(),
                        strbuf.toString(), e);
                throw new ConfigurationException(e);
            }
     ...
     * 
     * @param xmlobj
     *            The xmlobject to marshal
     * @param xmlconf
     *            This control file naming
     * @param directory
     *            The directory where to store the file
     * @throws Exception
     */
    public void createXMLFile(Object xmlobj, XMLCONFIG xmlconf, String directory)
            throws ConfigurationException {
        ...
        URL xsdUrl = ConfigFileManager.class.getClassLoader().getResource(
                xmlconf.xsd());
        if (xsdUrl == null) {
            LOGGER.error("Could not find xsd file {} in classpath",
                    xmlconf.xsd());
        ...
        try {
            ...
            LOGGER.error("Could not vaildate xml file {} with xsd file {}",
                    xmlconf.xml(), xmlconf.xsd(), e);
            throw new ConfigurationException(e);
        }
        ...
        File file = new File(directory + File.separator + xmlconf.xml());
        ...
        LOGGER.debug("Create new file in directory {} for xml object {}",
                directory, xmlconf.nametag());
    }
}
