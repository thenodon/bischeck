 ...
 * default is bischeck</li>
 * <li>
 * sendInternal - if true send the internal statistics, default is false.</li>
 * <li>
 * sendInternalInterval - the cron expression to define how often the data
 * should be sent.</li>
 * </ul>
 * 
 */
public class InternalSurveillance implements Job {
    private final static Logger LOGGER = LoggerFactory
            .getLogger(InternalSurveillance.class);
    private final OperatingSystemMXBean osMxBean = ManagementFactory
    ...
    public static void init() throws SchedulerException, ParseException {
        ...
        bischeckHostName = ConfigurationManager.getInstance().getProperties()
                .getProperty("bischeckHostName", "bischeck");
        boolean sendInternal = Boolean.valueOf(ConfigurationManager
                .getInstance().getProperties()
                .getProperty("sendInternal", "false"));
        String sendInternalInterval = ConfigurationManager.getInstance()
                .getProperties()
                .getProperty("sendInternalInterval", "0 */5 * * * ? *");
        ...
        if (sendInternal) {
            ...
            JobDetail job = newJob(InternalSurveillance.class)
                    ...
                    .withDescription("Internal").build();
            ...
            CronTrigger trigger = newTrigger()
                    ...
                    .forJob("Internal", "Internal").build();
    ...
    /**
     * Retrieve metrics timers and create a nagios performance data string for
     * execution timers mean value.
     * 
    ...
    public String executeTimers() {
        ...
        /*
         * MetricPredicate predicate = MetricPredicate.ALL; for (Entry<String,
         * SortedMap<MetricName, Metric>> entry :
         * getMetricsRegistry().getGroupedMetrics( predicate ).entrySet()) {
         * 
         * for (Entry<MetricName, Metric> subEntry :
         * entry.getValue().entrySet()) { Timer timer = ((Timer)
         * subEntry.getValue());
         * 
         * if (!entry.getKey().matches("com.ingby.socbox.bischeck.servers")) {
         * strbuf.append(trim(entry.getKey())); strbuf.append("_");
         * strbuf.append(subEntry.getKey().getName()); strbuf.append("=");
         * strbuf.append(Util.roundDecimals((float)timer.getMean()));
         * strbuf.append("ms;;;; "); } } strbuf.append(" "); }
         * 
    ...
    /*
     * private MetricsRegistry getMetricsRegistry() { MetricsRegistry metricsMap
    ...
    public void execute(JobExecutionContext arg0) throws JobExecutionException {
        ServerMessageExecutor.getInstance().executeInternal(bischeckHostName,
                "bischeck-timers", NAGIOSSTAT.OK, this.executeTimers());
        ServerMessageExecutor.getInstance().executeInternal(bischeckHostName,
                "bischeck-tps", NAGIOSSTAT.OK, this.tpsTimers());
        ServerMessageExecutor.getInstance().executeInternal(bischeckHostName,
                "bischeck-jvm", NAGIOSSTAT.OK, this.jvm());
    }
